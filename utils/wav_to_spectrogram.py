...
from tensorflow.spectrogram_generator import SpectrogramGenerator
...
def directory_to_spectrograms(args):
    ...
    while True:
        ...
        try:
            ...
            if i % 1000 == 0:
                logger.debug("Processed {} images".format(i))

        except StopIteration:
            logger.debug("Stopped on {}".format(language))
            break
...
if __name__ == "__main__":
    config = load(open("../tensorflow/config.yaml", "rb"), Loader=FullLoader)
