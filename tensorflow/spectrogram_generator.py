...
class SpectrogramGenerator(object):
    ...
    def audioToSpectrogram(self, file, pixel_per_sec, height, width):
        ...
        command = "sox -V0 '{}' -n remix 1 rate 10k spectrogram -y {} -x {} -X {}  -m -r -o {}".format(file, height,
                                                                                                       width,
                                                                                                       pixel_per_sec,
                                                                                                       file_name)
        ...
        if errors:
            logger.debug("SpectrogramGenerator Exception: ", errors)
            ...
            try:
                ...
                for i in range(0, num_segments):
                    ...
                    if slice.max() == 0 and slice.min() == 0:
                        logger.debug("Ignored a black image.")
                        continue

                    yield slice

            except Exception as e:
                logger.exception("SpectrogramGenerator Exception: ", e, file)
                pass
